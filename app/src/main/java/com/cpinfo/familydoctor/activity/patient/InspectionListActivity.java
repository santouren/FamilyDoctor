...
import com.cpinfo.familydoctor.adapter.InspectionListAdapter;
 ...
 * 居民版=》检查列表
 */
public class InspectionListActivity extends BaseActivity implements View.OnClickListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    private List<InspectionListBean.DataBean> inspectionListData;
    ...
    private void getData(boolean isShowLoading) {
        ...
        if (!TextUtils.isEmpty(identityId)) {
            ApiRequestManager.getInspectionList(identityId, startItem, endItem, new RequestCallBack<InspectionListBean>() {
                @Override
                public void onSuccess(InspectionListBean data) {
                    inspectionListData = data.getData();
                    if (!isLoadMore && !isRefresh) {
                        initView();
                    } else if (isRefresh) {
                        isRefresh = false;
                        inspectionListAdapter.setNewData(inspectionListData);
                        ptrFrameLayout.refreshComplete();
                    } else {//请求加载更多数据
                        inspectionListAdapter.addData(inspectionListData);
    ...
    private void initView() {
        if (inspectionListData.size() <= 0) {
            showErrorView("您没有检查记录！", R.drawable.ic_empty);
            return;
        }

        inspectionList.setLayoutManager(new LinearLayoutManager(this));
        inspectionListAdapter = new InspectionListAdapter(mContext, inspectionListData);
        inspectionListAdapter.setOnLoadMoreListener(this, inspectionList);
        inspectionListAdapter.openLoadAnimation(BaseQuickAdapter.SLIDEIN_LEFT);
        inspectionListAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                //跳转到检查详情页面
                InspectionListBean.DataBean inspectionItem = (InspectionListBean.DataBean) adapter.getData().get(position);
                Intent intent = new Intent(mContext, InspectionDetailActivity.class);
                intent.putExtra("applyid", inspectionItem.getApplyid());// 病人id
                intent.putExtra("jcbh", inspectionItem.getAccno());
                intent.putExtra("region", inspectionItem.getExcdeptname());
                if ("影像".equals(inspectionItem.getApplytype())) {
                    intent.putExtra("cloudType", "云影像");
                } else if ("心电".equals(inspectionItem.getApplytype())) {
                    intent.putExtra("cloudType", "云心电");
                    intent.putExtra("cloudAddress", inspectionItem.getXd_url());
                }
                startActivity(intent);
            }
        });
        inspectionList.setAdapter(inspectionListAdapter);
        showContentView();
    }
    ...
    public void onLoadMoreRequested() {
        if (inspectionListData.size() >= 10) {
            startItem += 10;
            endItem += 10;
            isLoadMore = true;
            getData(false);
        } else {
            inspectionListAdapter.loadMoreEnd(false);
        }
    }
}
